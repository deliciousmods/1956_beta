#!/usr/bin/env python3
"""
reprocess_states.py - State Name Localization Processor

USAGE:
    python reprocess_states.py              # Process and overwrite
    python reprocess_states.py --dry-run    # Preview changes only
    python reprocess_states.py --audit=log  # Save changes to CSV log

File location: <script_dir>/localisation/english/state_names_l_english.yml
"""
...
LOC_LINE_RE = re.compile(
    r'^(?P<indent>\s*)(?:(?P<tag>[A-Z0-9]{2,5})_)?STATE_(?P<id>\d+):(?P<suf>\d+)\s+"(?P<text>[^"]*)"\s*(?:#(?P<comment>.*))?$'
)
...
def parse_file(text: str):
    """Parse localization file into headers and entries by State ID."""
    headers = []
...
def expand_entries(groups, memberships, entries, audit=None):
    ...
    for sid, elist in entries.items():
        ...
        for src in list(by_tag.keys()):
            ...
            for tgt in all_tags - existing:
                ...
                _add_entry(sid, tgt, by_tag[best], entries, existing, audit, 'strict')
                added += 1
        ...
        for gname, sources in group_sources.items():
            ...
            for tgt in groups[gname].tags - existing:
                ...
                _add_entry(sid, tgt, by_tag[best], entries, existing, audit, 'group')
                added += 1
        
    return added


def _add_entry(sid, tag, src_entry, all_entries, existing_set, audit, mode):
    """Create and append new entry."""
    indent = src_entry['indent']
    comment_part = f" # {src_entry['comment']}" if src_entry['comment'] else ''
    line = f"{indent}{tag}_STATE_{sid}:{src_entry['suffix']} \"{src_entry['text']}\"{comment_part}"
    
    all_entries[sid].append({
        'tag': tag, 'id': sid, 'suffix': src_entry['suffix'],
    ...
    if audit:
        audit.writerow([sid, tag, src_entry['tag'], src_entry['text'], mode])
...
def build_output(headers, entries, groups, memberships):
    ...
    for sid in entries.keys():
        for e in entries[sid]:
            ...
            if group_name:
                grouped_entries[group_name].append((sid, e))
            else:
                ungrouped_entries.append((sid, e))
    
    lines = list(headers)
    
    if ungrouped_entries:
        ungrouped_entries.sort(key=lambda x: x[0])
        for sid, e in ungrouped_entries:
            comment_part = f" # {e['comment']}" if e['comment'] else ''
            line = f" {e['tag']+'_' if e['tag'] else ''}STATE_{e['id']}:{e['suffix']} \"{e['text']}\"{comment_part}"
            lines.append(line)
    ...
    for group_name in group_order:
        ...
        for sid, e in group_list:
            comment_part = f" # {e['comment']}" if e['comment'] else ''
            if e.get('tag'):
                line = f"{e['indent']}{e['tag']}_STATE_{e['id']}:{e['suffix']} \"{e['text']}\"{comment_part}"
            else:
                line = f" STATE_{e['id']}:{e['suffix']} \"{e['text']}\"{comment_part}"
...
# ---------------------------
def main():
    parser = argparse.ArgumentParser(description='HOI4 State Name Localization Processor', add_help=False)
    ...
    target_file = script_dir / "localisation" / "english" / "state_names_l_english.yml"
    
    if not target_file.exists():
        print(f"ERROR: File not found: {target_file}")
        print("Expected structure: <script_location>/localisation/english/state_names_l_english.yml")
        sys.exit(1)
    ...
    print(f"Parsed {len(entries)} STATE IDs")
    ...
    if args.audit:
        ...
        audit_writer.writerow(['sid', 'target', 'source', 'text', 'mode'])
